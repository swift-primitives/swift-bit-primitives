...
extension Bit.Index.Location<UInt>.Test.Unit {
    ...
    func initFromComponents() {
        let word = Index<UInt64>(__unchecked: (), Ordinal.Position(5))
    ...
    func initFromWordAndBit() {
        let word = Index<UInt64>(__unchecked: (), Ordinal.Position(0))
    ...
    func initFromBitIndex() {
        ...
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(70))
        let location = Bit.Index.Location<UInt64>(index: bitIndex, bitsPerWord: .bitWidth)

        #expect(location.word.position.rawValue == 1)
    ...
    func initFromBitCount() {
        ...
        let count = Bit.Index.Count(Cardinal.Count(70))
    ...
    func locationForBitIndexZero() {
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(0))
        let location = Bit.Index.Location<UInt64>(index: bitIndex, bitsPerWord: .bitWidth)

        #expect(location.word.position.rawValue == 0)
    ...
    func locationForBitIndex63() {
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(63))
        let location = Bit.Index.Location<UInt64>(index: bitIndex, bitsPerWord: .bitWidth)
    ...
    func locationForBitIndex64() {
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(64))
        let location = Bit.Index.Location<UInt64>(index: bitIndex, bitsPerWord: .bitWidth)
    ...
    func uint8WordType() {
        ...
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(10))
    ...
    func uint32WordType() {
        ...
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(35))
...
extension Bit.Index.Location<UInt>.Test.EdgeCase {
    ...
    func boundaryMaxBitUInt8() {
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(7))
        let location = Bit.Index.Location<UInt8>(index: bitIndex, bitsPerWord: .bitWidth)
    ...
    func boundaryWordTransitionUInt8() {
        ...
        let bit7 = Bit.Index(__unchecked: (), Ordinal.Position(7))
        let bit8 = Bit.Index(__unchecked: (), Ordinal.Position(8))
        ...
        #expect(loc7.word.position.rawValue == 0)
        #expect(loc7.bit.rawValue.rawValue == 7)

        #expect(loc8.word.position.rawValue == 1)
        #expect(loc8.bit.rawValue.rawValue == 0)
    }

    @Test("large bit index")
    func largeBitIndex() {
        let bitIndex = Bit.Index(__unchecked: (), Ordinal.Position(1000))
        let location = Bit.Index.Location<UInt64>(index: bitIndex, bitsPerWord: .bitWidth)

        // 1000 / 64 = 15, 1000 % 64 = 40
        #expect(location.word.position.rawValue == 15)
...
extension Bit.Index.Storage<UInt>.Test.Unit {
    ...
    func storageForZeroBits() {
        let count = Bit.Index.Count(Cardinal.Count(0))
        let storage = Bit.Index.Storage<UInt64>(count: count, bitsPerWord: .bitWidth)

        #expect(storage.wordCount.count.rawValue == 0)
        #expect(storage.unusedBits.count.rawValue == 0)
    }

    @Test("storage for exactly 64 bits")
    func storageForExactly64Bits() {
        let count = Bit.Index.Count(Cardinal.Count(64))
        let storage = Bit.Index.Storage<UInt64>(count: count, bitsPerWord: .bitWidth)

        #expect(storage.wordCount.count.rawValue == 1)
        #expect(storage.unusedBits.count.rawValue == 0)
    }

    @Test("storage for 65 bits")
    func storageFor65Bits() {
        let count = Bit.Index.Count(Cardinal.Count(65))
        let storage = Bit.Index.Storage<UInt64>(count: count, bitsPerWord: .bitWidth)

        #expect(storage.wordCount.count.rawValue == 2)
        #expect(storage.unusedBits.count.rawValue == 63)
    }

    @Test("storage for 100 bits")
    func storageFor100Bits() {
        let count = Bit.Index.Count(Cardinal.Count(100))
        let storage = Bit.Index.Storage<UInt64>(count: count, bitsPerWord: .bitWidth)

        // 100 bits needs 2 words (128 bits capacity), with 28 unused
        #expect(storage.wordCount.count.rawValue == 2)
        #expect(storage.unusedBits.count.rawValue == 28)
    }

    @Test("storage for UInt8 words")
    func storageForUInt8Words() {
        let count = Bit.Index.Count(Cardinal.Count(10))
        let storage = Bit.Index.Storage<UInt8>(count: count, bitsPerWord: .bitWidth)

        // 10 bits needs 2 bytes (16 bits capacity), with 6 unused
        #expect(storage.wordCount.count.rawValue == 2)
        #expect(storage.unusedBits.count.rawValue == 6)
    }

    @Test("capacity-based init")
    func capacityBasedInit() {
        let capacity = Bit.Index.Count(Cardinal.Count(100))
...
extension Bit.Index.Storage<UInt>.Test.EdgeCase {
    ...
    func storageForOneBit() {
        let count = Bit.Index.Count(Cardinal.Count(1))
        let storage = Bit.Index.Storage<UInt64>(count: count, bitsPerWord: .bitWidth)
    ...
    func storageAtExactWordBoundary() {
        let count = Bit.Index.Count(Cardinal.Count(128))
        let storage = Bit.Index.Storage<UInt64>(count: count, bitsPerWord: .bitWidth)
